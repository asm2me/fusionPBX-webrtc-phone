...
/*
	...
	Installation: Add to your FusionPBX theme footer (e.g., resources/footer.php) before </body>:
	  <?php if (file_exists($_SERVER['DOCUMENT_ROOT'].'/app/web_phone2/webrtc_phone_inc.php')) { include $_SERVER['DOCUMENT_ROOT'].'/app/web_phone2/webrtc_phone_inc.php'; } ?>
*/
...
if (isset($_SESSION['user_uuid']) && permission_exists('webrtc_phone_view')) {
	...
	if ($webrtc_enabled === 'true') {
		...
		// Load our translations fresh (isolated from any existing $text)
		$_webrtc_text = [];
		...
		// Always output - even en-us ensures t() works; include detected lang for debugging
