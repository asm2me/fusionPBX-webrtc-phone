...
if ($action === 'create' && permission_exists('click_to_dial_edit')) {
	...
	if (empty($extension_uuid)) {
		...
		$sql .= "allowed_origins, button_color, button_position, button_label, token_enabled, ";
		$sql .= "insert_date, insert_user) ";
		$sql .= "VALUES (:uuid, :domain_uuid, :extension_uuid, :api_token, :token_name, ";
		$sql .= ":allowed_origins, :button_color, :button_position, :button_label, 'true', ";
		$sql .= "now(), :user_uuid) ";
...
//get available extensions for the dropdown
...
.ctd-token-str { font-family: monospace; font-size: 11px; word-break: break-all; max-width: 200px; display: inline-block; }
...
<div class="ctd-setup">
	...
	<p style="color:#666; margin-bottom:20px;">Generate embeddable JavaScript widgets that allow third-party websites to make calls through your PBX.</p>
		...
		<?php else: ?>
			<table class="ctd-table">
				...
				<?php foreach ($tokens as $tok): ?>
					<tr>
						<td><strong><?php echo htmlspecialchars($tok['token_name'] ?? 'Unnamed'); ?></strong></td>
						<td>
						...
						<td>
							<?php
								$origins = trim($tok['allowed_origins'] ?? '');
								echo $origins ? '<small>' . nl2br(htmlspecialchars($origins)) . '</small>' : '<small style="color:#888">Any</small>';
							?>
						</td>
						<td>
							<span class="ctd-color-preview" style="background:<?php echo htmlspecialchars($tok['button_color'] ?? '#1a73e8'); ?>"></span>
	...
	<!-- Usage Instructions -->
	<div class="ctd-card">
		<h3 style="margin-top:0">How to Install on Your Website</h3>
		<ol style="font-size:14px; line-height:1.8; color:#555;">
			<li>Create a token above by selecting an extension and configuring the appearance.</li>
			...
			<li>The phone widget will automatically appear on your site.</li>
		</ol>

		<h3>Click-to-Dial Links</h3>
		<p style="font-size:14px; color:#555;">The plugin automatically detects phone numbers on your page. You can also create explicit dial links:</p>
		<div class="ctd-embed-box">
			<pre class="ctd-embed-code">&lt;!-- Automatic: tel: links are auto-detected --&gt;
&lt;a href="tel:+15551234567"&gt;Call Us&lt;/a&gt;

&lt;!-- Explicit: use data-ctd-dial attribute --&gt;
&lt;button data-ctd-dial="1001"&gt;Call Sales&lt;/button&gt;
&lt;a href="#" data-ctd-dial="1002"&gt;Call Support&lt;/a&gt;

&lt;!-- Auto-dial on load (e.g. callback page) --&gt;
&lt;div data-ctd-auto-dial="1001"&gt;&lt;/div&gt;</pre>
		</div>
	</div>
</div>
