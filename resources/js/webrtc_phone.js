...
var WebRTCPhone = (function () {
	...
	// --- Build Version ---
	var BUILD_VERSION = '1.3.0-b20260407';
	...
	function renderHistoryPanel() {
		...
		if (state.callHistory.length === 0) {
			...
			for (var i = 0; i < state.callHistory.length; i++) {
				...
				if (!ticketInfo) {
					html += '<button class="webrtc-history-report-btn" onclick="event.stopPropagation();WebRTCPhone.openTicketReport(' + i + ')" title="' + escapeHtml(t('reportIssue')) + '">Report</button>';
				}
